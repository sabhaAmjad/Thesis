...
import scala.collection.mutable.Queue
  ...
  class Kafka_Prod_Log_Slid(fileName : String) {
    ...
    def prod_log_slid ( topic : String  , p : Long, first_line: java.time.LocalDate , lastline :java.time.LocalDate )  {
      try {
        ...
        while (fixwind_weblogs.isBefore(window_date_1) == true && (cond == true)){
        ...
        while ( (window_date_0_5.isBefore(lastline) == true) && (cond == true)  && (newWindowCount < 2*TotalWindows )) {
          ...
          println("next record")
          val actual_date = println(fixwind_weblogs)
          if ((fixwind_weblogs.isAfter(window_date_0_5) && fixwind_weblogs.isBefore(window_date_1_5)) == true) {
            println("between ")
            producer.send(new ProducerRecord[String, String](topic, s"$window", s"$record"))
            println(s"Sent to odd: ${record.mkString} , ${window}")
          ...
          }else if (fixwind_weblogs.isEqual(window_date_0_5) == true) {
            println("equal t0")
            producer.send(new ProducerRecord[String, String](topic, s"$window", s"$record"))
            println(s"Sent to odd: ${record.mkString} , ${window}")
          ...
          else if (fixwind_weblogs.isEqual(window_date_1_5) == true) {
            println("equal t1")
            producer.send(new ProducerRecord[String, String](topic, s"$window", s"$record"))
            println(s"Sent to odd: ${record.mkString} , ${window}")
          ...
          if (fixwind_weblogs.isAfter(window_date_1_5) == true) {
            println(" next window")
        ...
        while (fixwind_weblogs.isAfter(window_date_2nd_last) == true && (cond == true)){
          println(fixwind_weblogs)
        if ((fixwind_weblogs.isAfter(window_date_2nd_last) && fixwind_weblogs.isBefore(window_date_last)) == true) {
          println("between ")
          producer.send(new ProducerRecord[String, String](topic, s"$window", s"$record"))
          println(s"Sent: ${record.mkString} , ${window}")
          newlineCount += 1
        ...
        } else if (fixwind_weblogs.isEqual(window_date_last) == true) {
          println("equal t1")
          producer.send(new ProducerRecord[String, String](topic, s"$window", s"$record"))
          println(s"Sent: ${record.mkString} , ${window}")
          newlineCount += 1
